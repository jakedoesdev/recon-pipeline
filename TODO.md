...
20. ~~**Selective pipeline phases**~~ — Done. `--phases` flag on `rp pipeline` accepts a comma-separated list of phases to run: `enum` (e), `resolve` (res), `reverse` (rev), `headers` (h), `tls` (t), `rdap` (r), `wpscan` (w). Full names and shorthand aliases both work. `scope` runs automatically for unscoped FQDNs, `analyze` runs automatically after any data-gathering phase. Omit `--phases` for default all-phases behavior. Replaces the old `--no-rdap` and `--no-wpscan` flags.
