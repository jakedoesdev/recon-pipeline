...
20. **Selective pipeline phases** — Allow the user to choose which phases `rp pipeline` runs (e.g. `--phases enum,resolve,reverse,headers`). All phases except `scope` and `analyze` are optional and individually selectable. `scope` always runs for new FQDNs and `analyze` always runs after new information has been gathered — these two are automatic and not skippable. Enables workflows like running only `enum` + `resolve` for a quick survey, or re-running just `wpscan` after getting an API key, without dropping to individual subcommands.
