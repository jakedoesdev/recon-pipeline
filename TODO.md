...
3. **Native scope enforcement after enum** — When `enum` or `pipeline` runs, check that allow.txt and deny.txt files were provided. `deny.txt` doesn't need entries, but `allow.txt` must have at least one domain. If not provided, offer to auto-generate an `allow.txt` with `*.rootdomain.tld` for every root domain in the user's targets file (root domains only, not `*.sub.rootdomain.tld`). Scope should run automatically after enum completes, before any downstream modules.

4. **Resilient crt.sh with deferred retry** — crt.sh frequently fails during enum. Track which target domains failed crt.sh and either retry them later in the flow or save them to a `rescan.txt` file for reprocessing. If a target can't be successfully queried before enum finishes, re-run crt.sh for those targets at the end and merge results into the store before continuing the pipeline. Goal: don't let transient crt.sh failures cause missed subdomains.
